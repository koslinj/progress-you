import { DayOfActivity } from "./ActivitiesSwiper"
    ...
    return (
        <div className="bg-slate-500 rounded-xl p-2 md:p-6 text-center">
            ...
            <div>
                ...
                {item.running && <p className=' text-lg md:text-2xl'>{Math.floor(item.minutes.running/60)}:{String(item.minutes.running % 60).padStart(2,'0')}</p>}
            </div>
            <div>
                <div className="flex justify-center gap-4 md:gap-8 items-center mt-4">
                ...
                {item.coding && <p className=' text-lg md:text-2xl'>{Math.floor(item.minutes.coding/60)}:{String(item.minutes.coding % 60).padStart(2,'0')}</p>}
            </div>
        </div>
    )
}
